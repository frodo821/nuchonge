...
├── dictionary.sqlite3    # 辞書データベース（SQLite3形式）
├── instruction.md        # 本プロジェクト指示書
├── sentences/            # 例文ディレクトリ
│   ├── 日常-1.1.md     # 例文ファイル
│   └── ...
└── lexicon/              # 辞書エントリーディレクトリ（マークダウン形式）
    ├── çit.md            # 単語エントリー
    └── ...
```

### データベース構造
辞書データベース（dictionary.sqlite3）は以下のテーブルから構成されています：
...
- **parts_of_speech**: 品詞情報
- **phonological_patterns**: 音韻型情報
- **inflection_types**: 活用・曲用の種類カテゴリ

詳細なデータベース利用方法は `cline_docs/databaseUsage.md` を参照してください。
...
- 修正時は関連する全てのエントリーを更新
- データベースとMarkdownファイルの両方で一貫性を維持

### 検索と索引

- 辞書データベース（dictionary.sqlite3）で効率的な検索を実行
- 活用形・曲用形からの逆引き検索にデータベースを活用
