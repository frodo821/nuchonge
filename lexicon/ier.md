# 見出し語: ier

## 文法情報
- **類別**: 動詞
- **音韻型**: 前舌
- **動詞分類**: 伝達動詞

## 語源情報
現代日本語の「言う(いう)」から変化。「いう」→「iu」→「ier」と音韻変化した。「u」が「er」に変化し、より明確な子音終わりの形態になった。

## 定義
1. 言う、述べる（言葉を発する）
2. 命じる、宣言する（特に権威ある立場から）
3. 伝える、伝達する（情報や思いを表現する）
4. 名づける、呼ぶ（何かの名称を述べる）

## 活用表

### 動詞「言う」の活用表

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>ierew</td>
      <td>ierep</td>
      <td>iere</td>
      <td>ierel</td>
      <td>ieres</td>
      <td>ieren</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>iertaw</td>
      <td>iertap</td>
      <td>ierta</td>
      <td>iertal</td>
      <td>iertas</td>
      <td>iertan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>ierow</td>
      <td>ierop</td>
      <td>iero</td>
      <td>ierol</td>
      <td>ieros</td>
      <td>ieron</td>
    </tr>
    <!-- 仮定法 -->
    <tr style="background-color: #f0fff0;">
      <th rowspan="3" style="background-color: #d9ffd9;">仮定法</th>
      <th>現在</th>
      <td>ierimw</td>
      <td>ierimp</td>
      <td>ierim</td>
      <td>ieriml</td>
      <td>ierims</td>
      <td>ierimn</td>
    </tr>
    <tr style="background-color: #f0fff0;">
      <th>過去</th>
      <td>ierm̩taw</td>
      <td>ierm̩tap</td>
      <td>ierm̩ta</td>
      <td>ierm̩tal</td>
      <td>ierm̩tas</td>
      <td>ierm̩tan</td>
    </tr>
    <tr style="background-color: #f0fff0;">
      <th>未来</th>
      <td>iermow</td>
      <td>iermop</td>
      <td>iermo</td>
      <td>iermol</td>
      <td>iermos</td>
      <td>iermon</td>
    </tr>
    <!-- 義務法 -->
    <tr style="background-color: #ffedff;">
      <th rowspan="2" style="background-color: #ffd9ff;">義務法</th>
      <th>現在</th>
      <td>ierbkew</td>
      <td>ierbkep</td>
      <td>ierbke</td>
      <td>ierbkel</td>
      <td>ierbkes</td>
      <td>ierbken</td>
    </tr>
    <tr style="background-color: #ffedff;">
      <th>過去</th>
      <td>ierbktaw</td>
      <td>ierbktap</td>
      <td>ierbkta</td>
      <td>ierbktal</td>
      <td>ierbktas</td>
      <td>ierbktan</td>
    </tr>
    <!-- 可能法 -->
    <tr style="background-color: #e6ffff;">
      <th rowspan="2" style="background-color: #c9f2f2;">可能法</th>
      <th>現在</th>
      <td>ierkmew</td>
      <td>ierkmep</td>
      <td>ierkme</td>
      <td>ierkmel</td>
      <td>ierkmes</td>
      <td>ierkmen</td>
    </tr>
    <tr style="background-color: #e6ffff;">
      <th>過去</th>
      <td>ierkmtaw</td>
      <td>ierkmtap</td>
      <td>ierkmta</td>
      <td>ierkmtal</td>
      <td>ierkmtas</td>
      <td>ierkmtan</td>
    </tr>
    <!-- 推定法 -->
    <tr style="background-color: #fff2e6;">
      <th rowspan="2" style="background-color: #ffe6c2;">推定法</th>
      <th>現在</th>
      <td>ierhzew</td>
      <td>ierhzep</td>
      <td>ierhze</td>
      <td>ierhzel</td>
      <td>ierhzes</td>
      <td>ierhzen</td>
    </tr>
    <tr style="background-color: #fff2e6;">
      <th>過去</th>
      <td>ierhztaw</td>
      <td>ierhztap</td>
      <td>ierhzta</td>
      <td>ierhztal</td>
      <td>ierhztas</td>
      <td>ierhztan</td>
    </tr>
    <!-- 希求法 -->
    <tr style="background-color: #f9e6ff;">
      <th rowspan="2" style="background-color: #f0c2ff;">希求法</th>
      <th>現在</th>
      <td>iertgew</td>
      <td>iertgep</td>
      <td>iertge</td>
      <td>iertgel</td>
      <td>iertges</td>
      <td>iertgen</td>
    </tr>
    <tr style="background-color: #f9e6ff;">
      <th>過去</th>
      <td>iertgtaw</td>
      <td>iertgtap</td>
      <td>iertgta</td>
      <td>iertgtal</td>
      <td>iertgtas</td>
      <td>iertgtan</td>
    </tr>
    <!-- 様態推量法 -->
    <tr style="background-color: #e6f9ff;">
      <th rowspan="2" style="background-color: #c2f0ff;">様態推量法</th>
      <th>現在</th>
      <td>iersjew</td>
      <td>iersjep</td>
      <td>iersje</td>
      <td>iersjel</td>
      <td>iersjes</td>
      <td>iersjen</td>
    </tr>
    <tr style="background-color: #e6f9ff;">
      <th>過去</th>
      <td>iersjtaw</td>
      <td>iersjtap</td>
      <td>iersjta</td>
      <td>iersjtal</td>
      <td>iersjtas</td>
      <td>iersjtan</td>
    </tr>
    <!-- 伝聞法 -->
    <tr style="background-color: #e6ffe6;">
      <th rowspan="2" style="background-color: #c2ffc2;">伝聞法</th>
      <th>現在</th>
      <td>ierskew</td>
      <td>ierskep</td>
      <td>ierske</td>
      <td>ierskel</td>
      <td>ierskes</td>
      <td>iersken</td>
    </tr>
    <tr style="background-color: #e6ffe6;">
      <th>過去</th>
      <td>iersktaw</td>
      <td>iersktap</td>
      <td>ierskta</td>
      <td>iersktal</td>
      <td>iersktas</td>
      <td>iersktan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff0f0;">
      <th rowspan="1" style="background-color: #ffd9d9;">命令法</th>
      <th>命令</th>
      <td>-</td>
      <td>iera</td>
      <td>ieræ</td>
      <td>ierat</td>
      <td>ieras</td>
      <td>ierær</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #fffff0;">
      <th rowspan="2" style="background-color: #ffffd9;">条件法</th>
      <th>現在</th>
      <td>ierrew</td>
      <td>ierrep</td>
      <td>ierre</td>
      <td>ierrel</td>
      <td>ierres</td>
      <td>ierren</td>
    </tr>
    <tr style="background-color: #fffff0;">
      <th>過去</th>
      <td>ierretaw</td>
      <td>ierretap</td>
      <td>ierreta</td>
      <td>ierreal</td>
      <td>ierretas</td>
      <td>ierretan</td>
    </tr>
    <!-- 受動形 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="3" style="background-color: #f2d9f2;">受動形</th>
      <th>現在</th>
      <td>-</td>
      <td>-</td>
      <td>iertre</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>過去</th>
      <td>-</td>
      <td>-</td>
      <td>iertrta</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>未来</th>
      <td>-</td>
      <td>-</td>
      <td>iertro</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    </tr>
  </tbody>
</table>

### 特殊形式

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 150px;">形式</th>
    <th style="background-color: #e6e6e6; width: 200px;">機能</th>
    <th style="background-color: #e6e6e6;">例</th>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <td>iertas</td>
    <td>権威・宣言の過去形</td>
    <td>kama iertas (神が言った)</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <td>iertmow</td>
    <td>思考・内省を表す形</td>
    <td>iertmow (〜と思う)</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <td>ierskre</td>
    <td>伝聞形</td>
    <td>ierskre (〜と言われている)</td>
  </tr>
</table>

## 例文
1. "çkari, kagajke." kama iertas, çkari sɔfto kagajktrta. (「光よ、輝き出よ。」神が言われると、光がさっとさしてきました。)
2. kama mata ierta: "rkad, arjr spno kas, spno ari ʃitsr, mio tskr tr̩a haee." (神はまた言われました。「陸地には、あらゆる種類の草、種のある植物、実のなる木が生えよ。」)
3. vtʃi ierrow, ierta mani. (私が言うでしょう、言った通りに。)

## 関連語
- nam (名づける) - 動詞
- kotba (言葉) - 名詞
- to (と) - 接続詞
- kik (聞く) - 動詞

## 備考
「ier」は単なる発話行為を表すだけでなく、特に宗教的・創造的文脈では強い力を持つ言葉の発信を意味する。「ier-tas」という形式は特に神の言葉が現実を形作る創造行為を表すために使われる。また日常会話では引用標識「to」と組み合わせて使用されることが多い。
