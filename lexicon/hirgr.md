# 見出し語: hirgr

## 文法情報
- **類別**: 動詞
- **音韻型**: 前舌
- **動詞分類**: 前舌動詞

## 語源情報
現代日本語の「広がる」(ひろがる)から。「ひろ」の子音が保持され、母音は前舌化し、「がる」は「gr」に縮約された。

## 定義
1. 空間的に拡張する、範囲が大きくなる
2. (生物が)数を増やして領域を拡大する
3. 影響や効果が及ぶ範囲が大きくなる
4. 知識や考えが普及する

## 活用表

<h3>hirgr（広がる）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>hirгrew</td>
      <td>hirgrep</td>
      <td>hirgre</td>
      <td>hirgrel</td>
      <td>hirgres</td>
      <td>hirgred</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>hirgrtaw</td>
      <td>hirgrtap</td>
      <td>hirgrta</td>
      <td>hirgrtal</td>
      <td>hirgrtas</td>
      <td>hirgrtan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>hirgrow</td>
      <td>hirgrop</td>
      <td>hirgro</td>
      <td>hirgrol</td>
      <td>hirgros</td>
      <td>hirgron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>hirgrow</td>
      <td>hirgrop</td>
      <td>hirgro</td>
      <td>hirgrol</td>
      <td>hirgros</td>
      <td>hirgron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>hirgrotaw</td>
      <td>hirgrotap</td>
      <td>hirgrota</td>
      <td>hirgrotal</td>
      <td>hirgrotas</td>
      <td>hirgrotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（-jo-）</th>
      <th>命令形</th>
      <td>—</td>
      <td>hirgrjop</td>
      <td>hirgrjo</td>
      <td>—</td>
      <td>hirgrjos</td>
      <td>hirgrjon</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>hirgrbkew</td>
      <td>hirgrbkep</td>
      <td>hirgrbke</td>
      <td>hirgrbkel</td>
      <td>hirgrbkes</td>
      <td>hirgrbken</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>hirgkmew</td>
      <td>hirgkmep</td>
      <td>hirgkme</td>
      <td>hirgkmel</td>
      <td>hirgkmes</td>
      <td>hirgkmen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>hirgrhzew</td>
      <td>hirgrhzep</td>
      <td>hirgrhze</td>
      <td>hirgrhzel</td>
      <td>hirgrhzes</td>
      <td>hirgrhzen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>hirgrtgew</td>
      <td>hirgrtgep</td>
      <td>hirgrtge</td>
      <td>hirgrtgel</td>
      <td>hirgrtges</td>
      <td>hirgrtgen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>hirgrsjew</td>
      <td>hirgrsjep</td>
      <td>hirgrsje</td>
      <td>hirgrsjel</td>
      <td>hirgrsjes</td>
      <td>hirgrsjen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>hirgrskew</td>
      <td>hirgrskep</td>
      <td>hirgrske</td>
      <td>hirgrskel</td>
      <td>hirgrskes</td>
      <td>hirgrsken</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>hirgrsme</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>hirgrtre</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>hirgrtke</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>hirgrdse</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>hirgrve</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>hirgrsmta</td>
    <td style="font-style: italic;">広がってしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>hirgrtro</td>
    <td style="font-style: italic;">広がっているだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>hirgrdsta</td>
    <td style="font-style: italic;">広がり始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>hirgrsmbe</td>
    <td style="font-style: italic;">広がってしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>hirgrsmro</td>
    <td style="font-style: italic;">広がってしまえば</td>
  </tr>
</table>

## 例文
1. tr̩n̩ fue hirgr-te (地に増え広がれ)
2. ʃitsr-ri tr̩-d hirgr-ta-n (植物たちは地に広がった)
3. kono koten-a tr̩-ne smkr̩smdn̩ hirgr-ta (この考え方は世界の隅々まで広がった)
4. mz̩a tr̩-ne uæ-d hirgr-e (水が地の上に広がっている)

## 関連語
- **fue**: 増える
- **ɔir**: 覆う
- **atsmar**: 集まる (対義語として)
- **tszk**: 続く

## 備考
創造物語において、hirgr（広がる）は神の祝福と命令の中核をなす概念として登場する。特に、生物が「増えて広がる」ことは、創造の目的の一つとして示されている。空間的な拡大だけでなく、数の増加と共に領域を拡大していくという概念を含んでおり、創造された世界を満たしていくという神の計画を表している。
