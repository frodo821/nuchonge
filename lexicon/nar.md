# 見出し語: nar

## 文法情報
- **類別**: 動詞
- **音韻型**: 後舌
- **動詞分類**: 変化動詞

## 語源情報
現代日本語の「なる」から変化。「なる」→「naru」→「nar」と音韻変化した。語末母音が脱落し、語幹のみの形態になった。

## 定義
1. なる、変化する（ある状態から別の状態への変化）
2. 生じる、発生する（新たに出現する）
3. 成る、達成する（実現・完成に至る）
4. 結果として〜となる（結果や帰結を表す）

## 活用表

<h3>nar（なる）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>naraw</td>
      <td>narap</td>
      <td>nara</td>
      <td>naral</td>
      <td>naras</td>
      <td>naran</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>nartaw</td>
      <td>nartap</td>
      <td>narta</td>
      <td>nartal</td>
      <td>nartas</td>
      <td>nartan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>narrow</td>
      <td>narrop</td>
      <td>narro</td>
      <td>narrol</td>
      <td>narros</td>
      <td>narron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>narrow</td>
      <td>narrop</td>
      <td>narro</td>
      <td>narrol</td>
      <td>narros</td>
      <td>narron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>narrotaw</td>
      <td>narrotap</td>
      <td>narrota</td>
      <td>narrotal</td>
      <td>narrotas</td>
      <td>narrotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（特殊形）</th>
      <th>命令形</th>
      <td>—</td>
      <td>nare</td>
      <td>nare</td>
      <td>—</td>
      <td>naren</td>
      <td>naren</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>narbkaw</td>
      <td>narbkap</td>
      <td>narbka</td>
      <td>narbkal</td>
      <td>narbkas</td>
      <td>narbkan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>narkmaw</td>
      <td>narkmap</td>
      <td>narkma</td>
      <td>narkmal</td>
      <td>narkmas</td>
      <td>narkman</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>narhzaw</td>
      <td>narhzap</td>
      <td>narhza</td>
      <td>narhzal</td>
      <td>narhzas</td>
      <td>narhzan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>nartgaw</td>
      <td>nartgap</td>
      <td>nartga</td>
      <td>nartgal</td>
      <td>nartgas</td>
      <td>nartgan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>narsjaw</td>
      <td>narsjap</td>
      <td>narsja</td>
      <td>narsjal</td>
      <td>narsjas</td>
      <td>narsjan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>narskaw</td>
      <td>narskap</td>
      <td>narska</td>
      <td>narskal</td>
      <td>narskas</td>
      <td>narskan</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>narsma</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>nartra</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>nartka</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>nardsa</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>narva</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>narsmta</td>
    <td style="font-style: italic;">なってしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>nartro</td>
    <td style="font-style: italic;">なっているだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>nardsta</td>
    <td style="font-style: italic;">なり始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>narsmbka</td>
    <td style="font-style: italic;">なってしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>narsmro</td>
    <td style="font-style: italic;">なってしまえば</td>
  </tr>
</table>

## 例文
1. "majaa uæʃtan̩ wakrte, sora to umn̩ nare" to kama iertas. (「もやは上下に分かれ、空と海になれ」と神が言われました。)
2. sorano ʃta m̩za atsmarte umn̩ nare. (空の下の水は集まって海となり。)
3. tæmi çti çaa narta. (時は一日目になりました。)
4. iedɔrn̩ natta. (そのとおりになりました。)

## 関連語
- ar (ある) - 動詞
- owat (終わる) - 動詞
- wakr (区別する) - 動詞
- kætr̩ (変化) - 名詞

## 備考
「nar」は状態変化を表す基本的な動詞であり、特に創造神話において重要な役割を果たす。神の言葉によって何かが別のものに「なる」という創造の過程を表現する際に頻繁に使用される。命令形「nare」は特に創造の命令として強い力を持つ。また助動詞的な用法もあり、「〜することになる」「〜になっていく」といった文法的な機能も担う。
