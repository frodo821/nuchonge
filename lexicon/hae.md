# 見出し語: hae

## 文法情報
- **類別**: 動詞
- **音韻型**: 後舌
- **動詞分類**: 後舌動詞

## 語源情報
現代日本語の「生える」(はえる)から。「は」の子音が保持され、母音は後舌化し、「える」は「e」に縮約された。

## 定義
1. 植物が地面から出て成長する
2. 髪や毛、爪などが体から自然に出てくる
3. (比喩的に)何かが自然に出現する、発生する

## 活用表

<h3>hae（生える）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>haeaw</td>
      <td>haeap</td>
      <td>haea</td>
      <td>haeal</td>
      <td>haeas</td>
      <td>haean</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>haetaw</td>
      <td>haetap</td>
      <td>haeta</td>
      <td>haetal</td>
      <td>haetas</td>
      <td>haetan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>haerow</td>
      <td>haerop</td>
      <td>haero</td>
      <td>haerol</td>
      <td>haeros</td>
      <td>haeron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>haerow</td>
      <td>haerop</td>
      <td>haero</td>
      <td>haerol</td>
      <td>haeros</td>
      <td>haeron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>haerotaw</td>
      <td>haerotap</td>
      <td>haerota</td>
      <td>haerotal</td>
      <td>haerotas</td>
      <td>haerotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（-jo-）</th>
      <th>命令形</th>
      <td>—</td>
      <td>haejop</td>
      <td>haejo</td>
      <td>—</td>
      <td>haejos</td>
      <td>haejon</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>haebkaw</td>
      <td>haebkap</td>
      <td>haebka</td>
      <td>haebkal</td>
      <td>haebkas</td>
      <td>haebkan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>haekmaw</td>
      <td>haekmap</td>
      <td>haekma</td>
      <td>haekmal</td>
      <td>haekmas</td>
      <td>haekman</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>haehzaw</td>
      <td>haehzap</td>
      <td>haehza</td>
      <td>haehzal</td>
      <td>haehzas</td>
      <td>haehzan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>haetgaw</td>
      <td>haetgap</td>
      <td>haetga</td>
      <td>haetgal</td>
      <td>haetgas</td>
      <td>haetgan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>haesjaw</td>
      <td>haesjap</td>
      <td>haesja</td>
      <td>haesjal</td>
      <td>haesjas</td>
      <td>haesjan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>haeskaw</td>
      <td>haeskap</td>
      <td>haeska</td>
      <td>haeskal</td>
      <td>haeskas</td>
      <td>haeskan</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>haesma</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>haetra</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>haetka</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>haedsa</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>haeva</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>haesmta</td>
    <td style="font-style: italic;">生えてしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>haetro</td>
    <td style="font-style: italic;">生えているだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>haedsta</td>
    <td style="font-style: italic;">生え始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>haesmbka</td>
    <td style="font-style: italic;">生えてしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>haesmro</td>
    <td style="font-style: italic;">生えてしまえば</td>
  </tr>
</table>

## 例文
1. tr̃ad kas-a hae-a (地に草が生える)
2. ʃitsr-a m̃z-ñ hae-ro (植物は水に生えるだろう)
3. çit-ne kes-d kami-a hae-a (人の頭に髪が生える)
4. æno tno-d kinɔk-a hae-ta (家の庭にキノコが生えた)

## 関連語
- **kas**: 草
- **ʃitsr**: 植物
- **k**: 木
- **arwr**: 現れる

## 備考
