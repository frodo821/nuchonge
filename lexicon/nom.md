# 見出し語: nom

## 文法情報
- **類別**: 動詞
- **音韻型**: 後舌
- **動詞分類**: 他動詞

## 語源情報
現代日本語「のむ」(飲む)から変化
* /nomu/ → /nom/ と音韻変化
* 語末母音の脱落による単純化

## 定義
1. 液体を口に入れて飲み込む
2. 薬や栄養剤などを口から摂取する
3. （比喩的に）知識や情報を取り入れる
4. 煙や煙霧などを吸い込む
5. （感情・状況を）受け入れる、我慢する

## 活用表

<h3>nom（飲む）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>nomaw</td>
      <td>nomap</td>
      <td>noma</td>
      <td>nomal</td>
      <td>nomas</td>
      <td>noman</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>nomtaw</td>
      <td>nomtap</td>
      <td>nomta</td>
      <td>nomtal</td>
      <td>nomtas</td>
      <td>nomtan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>nomrow</td>
      <td>nomrop</td>
      <td>nomro</td>
      <td>nomrol</td>
      <td>nomros</td>
      <td>nomron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>nomrow</td>
      <td>nomrop</td>
      <td>nomro</td>
      <td>nomrol</td>
      <td>nomros</td>
      <td>nomron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>nomrotaw</td>
      <td>nomrotap</td>
      <td>nomrota</td>
      <td>nomrotal</td>
      <td>nomrotas</td>
      <td>nomrotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（-jo-）</th>
      <th>命令形</th>
      <td>—</td>
      <td>nomjop</td>
      <td>nomjo</td>
      <td>—</td>
      <td>nomjos</td>
      <td>nomjon</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>nombkaw</td>
      <td>nombkap</td>
      <td>nombka</td>
      <td>nombkal</td>
      <td>nombkas</td>
      <td>nombkan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>nomkmaw</td>
      <td>nomkmap</td>
      <td>nomkma</td>
      <td>nomkmal</td>
      <td>nomkmas</td>
      <td>nomkman</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>nomhzaw</td>
      <td>nomhzap</td>
      <td>nomhza</td>
      <td>nomhzal</td>
      <td>nomhzas</td>
      <td>nomhzan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>nomtgaw</td>
      <td>nomtgap</td>
      <td>nomtga</td>
      <td>nomtgal</td>
      <td>nomtgas</td>
      <td>nomtgan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>nomsjaw</td>
      <td>nomsjap</td>
      <td>nomsja</td>
      <td>nomsjal</td>
      <td>nomsjas</td>
      <td>nomsjan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>nomskaw</td>
      <td>nomskap</td>
      <td>nomska</td>
      <td>nomskal</td>
      <td>nomskas</td>
      <td>nomskan</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>nomsma</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>nomtra</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>nomtka</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>nomdsa</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>nomva</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>nomsmta</td>
    <td style="font-style: italic;">飲んでしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>nomtro</td>
    <td style="font-style: italic;">飲んでいるだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>nomdsta</td>
    <td style="font-style: italic;">飲み始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>nomsmbka</td>
    <td style="font-style: italic;">飲んでしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>nomsmro</td>
    <td style="font-style: italic;">飲んでしまえば</td>
  </tr>
</table>

## 例文
1. vtʃi m̃zo nomaw. (私は水を飲みます。)
2. çiti jaksno nomta. (人は薬を飲んだ。)
3. antp kino sakeo nomtap ka? (あなたは昨日酒を飲みましたか？)
4. kara nomtasma hño jome. (彼は飲み終わってから本を読む。)

## 関連語
- nommn: 飲み物（「飲む」+「もの」の複合語）
- nomdsm: 飲み始める（「飲む」+起動相「dsm」接尾辞）
- nomsm: 飲み切る（「飲む」+完遂「sm」接尾辞）
- tben: 食べる（関連基本動詞）
- tbenom: 食べ飲み（「食べる」+「飲む」の複合語）

## 備考
「nom」は日常的な基本動詞で、飲食に関する表現に必須です。後舌母音系動詞として、活用形に後舌母音が現れます。丁寧表現では「nommasaw」（飲みます）などの形が使われます。「jknom」（よく飲む、大酒飲みである）のように習慣・頻度を表す接頭辞「jk-」と組み合わせることも一般的です。

身体活動や生理現象を表す基本的な動詞として、様々なアスペクト標識と組み合わせて使用されます：
- nomtra (飲んでいる)
- nomsmta (飲んでしまった)
- nomtko (飲んでおこう)

比喩的には「知識を飲む」（知識を吸収する）、「怒りを飲む」（怒りを抑える）などの表現にも使用されます。また、「m̃z」（水）や「sake」（酒）などの液体名詞と共に使われることが多く、これらの名詞と組み合わせて複合語を形成することもあります（「m̃znom」（水飲み）、「sakenom」（酒飲み）など）。
