# 見出し語: nʒʃ

## 文法情報
- **類別**: 動詞
- **音韻型**: 前舌
- **動詞分類**: 前舌動詞

## 語源情報
現代日本語の「主」(ぬし)から。「ぬし」の子音が保持され、母音は削減され子音クラスタとなり「nʒʃ」となった。

## 定義
1. 何かの所有者や管理者になる
2. 権限を持って支配する、統治する
3. 責任を持って世話をする

## 活用表

<h3>nʒʃ（主になる）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>nʒʃew</td>
      <td>nʒʃep</td>
      <td>nʒʃe</td>
      <td>nʒʃel</td>
      <td>nʒʃes</td>
      <td>nʒʃen</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>nʒʃtaw</td>
      <td>nʒʃtap</td>
      <td>nʒʃta</td>
      <td>nʒʃtal</td>
      <td>nʒʃtas</td>
      <td>nʒʃtan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>nʒʃrow</td>
      <td>nʒʃrop</td>
      <td>nʒʃro</td>
      <td>nʒʃrol</td>
      <td>nʒʃros</td>
      <td>nʒʃron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>nʒʃrow</td>
      <td>nʒʃrop</td>
      <td>nʒʃro</td>
      <td>nʒʃrol</td>
      <td>nʒʃros</td>
      <td>nʒʃron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>nʒʃrotaw</td>
      <td>nʒʃrotap</td>
      <td>nʒʃrota</td>
      <td>nʒʃrotal</td>
      <td>nʒʃrotas</td>
      <td>nʒʃrotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（-jo-）</th>
      <th>命令形</th>
      <td>—</td>
      <td>nʒʃjop</td>
      <td>nʒʃjo</td>
      <td>—</td>
      <td>nʒʃjos</td>
      <td>nʒʃjon</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>nʒʃbkew</td>
      <td>nʒʃbkep</td>
      <td>nʒʃbke</td>
      <td>nʒʃbkel</td>
      <td>nʒʃbkes</td>
      <td>nʒʃbken</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>nʒʃkmew</td>
      <td>nʒʃkmep</td>
      <td>nʒʃkme</td>
      <td>nʒʃkmel</td>
      <td>nʒʃkmes</td>
      <td>nʒʃkmen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>nʒʃhzew</td>
      <td>nʒʃhzep</td>
      <td>nʒʃhze</td>
      <td>nʒʃhzel</td>
      <td>nʒʃhzes</td>
      <td>nʒʃhzen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>nʒʃtgew</td>
      <td>nʒʃtgep</td>
      <td>nʒʃtge</td>
      <td>nʒʃtgel</td>
      <td>nʒʃtges</td>
      <td>nʒʃtgen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>nʒʃsjew</td>
      <td>nʒʃsjep</td>
      <td>nʒʃsje</td>
      <td>nʒʃsjel</td>
      <td>nʒʃsjes</td>
      <td>nʒʃsjen</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>nʒʃskew</td>
      <td>nʒʃskep</td>
      <td>nʒʃske</td>
      <td>nʒʃskel</td>
      <td>nʒʃskes</td>
      <td>nʒʃsken</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>nʒʃsme</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>nʒʃtre</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>nʒʃtke</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>nʒʃdse</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>nʒʃve</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>nʒʃsmta</td>
    <td style="font-style: italic;">主になってしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>nʒʃtro</td>
    <td style="font-style: italic;">主になっているだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>nʒʃdsta</td>
    <td style="font-style: italic;">主になり始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>nʒʃsmbke</td>
    <td style="font-style: italic;">主になってしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>nʒʃsmro</td>
    <td style="font-style: italic;">主になってしまえば</td>
  </tr>
</table>

## 例文
1. antpraa sakana to tori to arjr dɔbutsuno nʒʃ aras (あなたがたは魚と鳥とすべての動物の主人です)
2. kamno kotbañ çita tr̃no nʒʃ-ta (神の言葉によって人は地の主人となった)
3. kono tɔçino nʒʃ-e-w (この土地の主人になります)
4. kamkamia tr̃-ne arjr ikimonono nʒʃ ara (神々は地のあらゆる生き物の主です)

## 関連語
- **ɔsamr**: 治める
- **mot**: 持つ
- **ataw**: 与える
- **ivæ**: 祝福する

## 備考
