# 見出し語: atsmar

## 文法情報
- **類別**: 動詞
- **音韻型**: 後舌
- **動詞分類**: 後舌動詞

## 語源情報
現代日本語の「集まる」(あつまる)から。「あつ」の子音が保持され、母音は後舌化し、「まる」は「mar」に縮約された。

## 定義
1. 散らばっていたものが一箇所に集合する
2. 複数の人や物が同じ場所に集合する
3. 資源や情報などが蓄積される

## 活用表

<h3>atsmar（集まる）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>atsmaraw</td>
      <td>atsmarap</td>
      <td>atsmara</td>
      <td>atsmaral</td>
      <td>atsmaras</td>
      <td>atsmaran</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>atsmartaw</td>
      <td>atsmartap</td>
      <td>atsmarta</td>
      <td>atsmartal</td>
      <td>atsmartas</td>
      <td>atsmartan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>atsmarrow</td>
      <td>atsmarrop</td>
      <td>atsmarro</td>
      <td>atsmarrol</td>
      <td>atsmarros</td>
      <td>atsmarron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>atsmarrow</td>
      <td>atsmarrop</td>
      <td>atsmarro</td>
      <td>atsmarrol</td>
      <td>atsmarros</td>
      <td>atsmarron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>atsmarrotaw</td>
      <td>atsmarrotap</td>
      <td>atsmarrota</td>
      <td>atsmarrotal</td>
      <td>atsmarrotas</td>
      <td>atsmarrotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（-jo-）</th>
      <th>命令形</th>
      <td>—</td>
      <td>atsmarjop</td>
      <td>atsmarjo</td>
      <td>—</td>
      <td>atsmarjos</td>
      <td>atsmarjon</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>atsmarbkaw</td>
      <td>atsmarbkap</td>
      <td>atsmarbka</td>
      <td>atsmarbkal</td>
      <td>atsmarbkas</td>
      <td>atsmarbkan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>atsmarkmaw</td>
      <td>atsmrkmap</td>
      <td>atsmrkma</td>
      <td>atsmrkmal</td>
      <td>atsmrkmas</td>
      <td>atsmrkman</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>atsmarhzaw</td>
      <td>atsmarhzap</td>
      <td>atsmarhza</td>
      <td>atsmarhzal</td>
      <td>atsmarhzas</td>
      <td>atsmarhzan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>atsmartgaw</td>
      <td>atsmartgap</td>
      <td>atsmartga</td>
      <td>atsmartgal</td>
      <td>atsmartgas</td>
      <td>atsmartgan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>atsmarsjaw</td>
      <td>atsmarsjap</td>
      <td>atsmarsja</td>
      <td>atsmarsjal</td>
      <td>atsmarsjas</td>
      <td>atsmarsjan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>atsmarskaw</td>
      <td>atsmarskap</td>
      <td>atsmarska</td>
      <td>atsmarskal</td>
      <td>atsmarskas</td>
      <td>atsmarskan</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>atsmarsma</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>atsmartra</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>atsmartka</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>atsmardsa</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>atsmarva</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>atsmarsmta</td>
    <td style="font-style: italic;">集まってしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>atsmartro</td>
    <td style="font-style: italic;">集まっているだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>atsmardsta</td>
    <td style="font-style: italic;">集まり始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>atsmarsmba</td>
    <td style="font-style: italic;">集まってしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>atsmarsmro</td>
    <td style="font-style: italic;">集まってしまえば</td>
  </tr>
</table>

## 例文
1. sorano ʃta m̩za atsmar-te umn̩ nar-e (空の下の水は集まって海になれ)
2. çit-ri tosn̩-d atsmar-ta-n (人々は図書館に集まった)
3. vtʃislno æ-d atsmar-ro-l (私たちの家に集まりましょう)
4. m̩za kubɔm-d atsmar-a (水はくぼみに集まる)

## 関連語
- **hirgr**: 広がる (対義語)
- **nar**: なる
- **tsukr**: 創る
- **um**: 海

## 備考
