# 見出し語: wakr

## 文法情報
- **類別**: 動詞
- **音韻型**: 後舌
- **動詞分類**: 行為動詞

## 語源情報
現代日本語の「分ける(わける)」から変化。「わける」→「waker」→「wakr」と音韻変化した。語中母音の「e」が脱落し、子音クラスタが形成された。

## 定義
1. 区別する、分ける（異なるものに区分する）
2. 分離する、切り離す（物理的に離す）
3. 区分する、区画する（空間や時間を分ける）
4. 配分する、割り当てる（何かを分配する）

## 活用表

<h3>wakr（分ける）の活用表</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e6e6ff;">
      <th colspan="2" rowspan="2"></th>
      <th colspan="3" style="text-align: center;">単数</th>
      <th colspan="3" style="text-align: center;">複数</th>
    </tr>
    <tr style="background-color: #e6e6ff;">
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
      <th>1人称</th>
      <th>2人称</th>
      <th>3人称</th>
    </tr>
  </thead>
  <tbody>
    <!-- 直説法 -->
    <tr style="background-color: #f0f0ff;">
      <th rowspan="3" style="background-color: #d9d9ff;">直説法</th>
      <th>現在</th>
      <td>wakraw</td>
      <td>wakrap</td>
      <td>wakra</td>
      <td>wakral</td>
      <td>wakras</td>
      <td>wakran</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>過去</th>
      <td>wakrtaw</td>
      <td>wakrtap</td>
      <td>wakrta</td>
      <td>wakrtal</td>
      <td>wakrtas</td>
      <td>wakrtan</td>
    </tr>
    <tr style="background-color: #f0f0ff;">
      <th>未来</th>
      <td>wakrrow</td>
      <td>wakrrop</td>
      <td>wakrro</td>
      <td>wakrrol</td>
      <td>wakrros</td>
      <td>wakrron</td>
    </tr>
    <!-- 条件法 -->
    <tr style="background-color: #f2fff2;">
      <th rowspan="2" style="background-color: #d9f2d9;">条件法<br>（-ro-）</th>
      <th>条件現在</th>
      <td>wakrrow</td>
      <td>wakrrop</td>
      <td>wakrro</td>
      <td>wakrrol</td>
      <td>wakrros</td>
      <td>wakrron</td>
    </tr>
    <tr style="background-color: #f2fff2;">
      <th>条件過去</th>
      <td>wakrrotaw</td>
      <td>wakrrotap</td>
      <td>wakrrota</td>
      <td>wakrrotal</td>
      <td>wakrrotas</td>
      <td>wakrrotan</td>
    </tr>
    <!-- 命令法 -->
    <tr style="background-color: #fff2f2;">
      <th rowspan="1" style="background-color: #f2d9d9;">命令法<br>（-jo-）</th>
      <th>命令形</th>
      <td>—</td>
      <td>wakrjop</td>
      <td>wakrjo</td>
      <td>—</td>
      <td>wakrjos</td>
      <td>wakrjon</td>
    </tr>
    <!-- その他の法 -->
    <tr style="background-color: #fff2ff;">
      <th rowspan="6" style="background-color: #f2d9f2;">その他の法</th>
      <th>義務法（-bk-）</th>
      <td>wakrbkaw</td>
      <td>wakrbkap</td>
      <td>wakrbka</td>
      <td>wakrbkal</td>
      <td>wakrbkas</td>
      <td>wakrbkan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>可能法（-km-）</th>
      <td>wakrkmaw</td>
      <td>wakrkmap</td>
      <td>wakrkma</td>
      <td>wakrkmal</td>
      <td>wakrkmas</td>
      <td>wakrkman</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>推定（-hz-）</th>
      <td>wakrhzaw</td>
      <td>wakrhzap</td>
      <td>wakrhza</td>
      <td>wakrhzal</td>
      <td>wakrhzas</td>
      <td>wakrhzan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>希求法（-tg-）</th>
      <td>wakrtgaw</td>
      <td>wakrtgap</td>
      <td>wakrtga</td>
      <td>wakrtgal</td>
      <td>wakrtgas</td>
      <td>wakrtgan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>様態推量（-sj-）</th>
      <td>wakrsjaw</td>
      <td>wakrsjap</td>
      <td>wakrsja</td>
      <td>wakrsjal</td>
      <td>wakrsjas</td>
      <td>wakrsjan</td>
    </tr>
    <tr style="background-color: #fff2ff;">
      <th>伝聞（-sk-）</th>
      <td>wakrskaw</td>
      <td>wakrskap</td>
      <td>wakrska</td>
      <td>wakrskal</td>
      <td>wakrskas</td>
      <td>wakrskan</td>
    </tr>
  </tbody>
</table>

<h3>相（アスペクト）形式（3人称単数現在形）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">完遂相（-sm-）</th>
    <td>wakrsma</td>
    <th style="background-color: #f2f2d9;">結果状態相（-tr-）</th>
    <td>wakrtra</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">先行準備相（-tk-）</th>
    <td>wakrtka</td>
    <th style="background-color: #f2f2d9;">起動相（-ds-）</th>
    <td>wakrdsa</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">終結相（-və-/-v-）</th>
    <td>wakrva</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>受動形（結果状態相の時制変化）</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">現在</th>
    <td>wakrtra</td>
    <th style="background-color: #f2f2d9;">過去</th>
    <td>wakrtrta</td>
  </tr>
  <tr style="background-color: #fffff0;">
    <th style="background-color: #f2f2d9;">未来</th>
    <td>wakrtro</td>
    <th style="background-color: #f2f2d9;"></th>
    <td></td>
  </tr>
</table>

<h3>代表的複合形式</h3>

<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse;">
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6; width: 200px;">過去完遂相</th>
    <td>wakrsmta</td>
    <td style="font-style: italic;">分けてしまった</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">未来結果状態相</th>
    <td>wakrtro</td>
    <td style="font-style: italic;">分かれているだろう</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">過去起動相</th>
    <td>wakrdsta</td>
    <td style="font-style: italic;">分け始めた</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">義務完遂相</th>
    <td>wakrsmbka</td>
    <td style="font-style: italic;">分けてしまわなければならない</td>
  </tr>
  <tr style="background-color: #f5f5f5;">
    <th style="background-color: #e6e6e6;">条件完遂相</th>
    <td>wakrsmro</td>
    <td style="font-style: italic;">分けてしまえば</td>
  </tr>
</table>

## 例文
1. karo mirte, kama tvjokso manskta, çkare jamikr̩ wakrta. (それを見て、神は大いに満足し、光と闇とを区別しました。)
2. "majaa uæʃtan̩ wakrte, sora to umn̩ nare" to kama iertas. (「もやは上下に分かれ、空と海になれ」と神が言われました。)
3. korske çiru to joro wakrtr̩owatte, kama jokso manskta. (こうして昼と夜を分け終えると、神は満足しました。)

## 関連語
- çiru (昼) - 名詞
- jor (夜) - 名詞
- uæʃ (上下) - 名詞
- owat (終わる) - 動詞
- nar (なる) - 動詞

## 備考
「wakr」は単に物理的な分離だけでなく、概念的な区別や分類も表す。特に創造神話において、混沌とした状態から秩序を生み出す行為として重要な意味を持つ。「wakr」による区別は、時間（昼と夜）、空間（上と下、空と海）、存在（光と闇）など、世界の基本的な二項対立を確立する行為として描かれる。
